<div wire:init="load">
        ...
        <div class="-my-5 divide-y">
            @foreach ($reservations as $reservation)
                <a href="{{ route('guest.reservation', [$reservation->slug]) }}">
                    <div class="-mx-5 cursor-pointer px-5 py-3 hover:bg-gray-50">
                        <div class="grid w-full grid-cols-3">
                            <div class="flex items-center space-x-5">
                                <div class="h-12 w-12 rounded-full bg-cover bg-center shadow-lg ring-1 ring-gray-400" style="background-image: url(/storage/{{ $reservation->property->photos()->first()->path }})"></div>
                                <div>
                                    <h2 class="text-lg">{{ $reservation->property->name }}</h2>
                                    <span class="text-muted text-sm">{{ $reservation->property->city }}, {{ $reservation->property->state }}</span>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center justify-end space-x-2">
                                <div class="rounded-lg bg-yellow-200 px-2 py-1 text-sm capitalize text-yellow-800">
                                    {{ $reservation->status }}
                                </div>
                                <div>
                                    <div>
    ...
    @else
        ...
        <div class="text-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto h-12 w-12 text-gray-300" width="40" height="40" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
