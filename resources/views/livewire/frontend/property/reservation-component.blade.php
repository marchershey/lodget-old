<div class="mt-10" x-data="{ open: false }" wire:init="load">
    ...
    @if ($showButton)
        <button wire:loading.remove wire:target="load" x-on:click="open = true" type="button" class="bg-primary hover:bg-primary-dark focus:ring-primary flex w-full items-center justify-center rounded-md border border-transparent px-8 py-3 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50">Select Dates</button>
    @else
        <div class="flex justify-center" wire:loading.flex wire:target="load">
            <svg class="text-muted h-[50px] w-[50px] animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <div x-show="open" x-cloak class="relative z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto">
            <div class="relative flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                <div x-on:click.away="open = false" class="relative w-full rounded-lg bg-gray-100 px-4 pt-5 pb-4 text-left sm:my-8 sm:w-full sm:max-w-sm sm:p-6">
                    <div class="absolute -top-2 -right-2 z-50">
                        <button x-on:click="open = false" class="rounded-full bg-gray-100 p-1">
                    ...
                    <div x-data="{
                        ...
                        <label for="guests" class="flex w-full items-center justify-between space-x-5">
                            <div class="flex flex-col">
                                <span class="text-sm font-semibold @error('guests') text-red-500 @enderror">Guests</span>
                                <span class="text-muted text-xs">How many guests will be staying?</span>
                            </div>
                            <div class="focus-within:ring-primary focus-within:border-primary group mt-1 flex w-full max-w-[150px] overflow-hidden rounded-md border border-gray-300 focus-within:ring-1 sm:text-sm @error('guests') border-red-500 @enderror">
                                ...
                                <input x-model.value="value" type="text" id="guests" class="w-full border-none bg-gray-50 px-1 py-1.5 text-center focus:outline-none focus:ring-0 group-focus-within:bg-white" tabindex="-1" readonly />
                    ...
                    <div class="mt-5 flex flex-col space-y-5 sm:mt-6">
                        <button wire:click="checkout()" type="button" class="bg-primary hover:bg-primary-dark focus:ring-primary inline-flex w-full justify-center rounded-md border border-transparent px-4 py-2 text-base font-medium text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2">Request Reservation</button>
                        <button wire:click="clearDates()" class="text-muted w-full rounded-md bg-gray-100">Clear Dates</button>
                    </div>
                </div>
            </div>
        </div>
